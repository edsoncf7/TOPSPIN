...
<head>
  ...
  <style>
    /* Copia aquí todo el CSS que ya tienes en tu archivo original */
    /* Reset básico */
    * {
    ...
    body {
    background-image: url('/img/1.jpg'); /* Cambia 'fondo.jpg' por el nombre real de tu imagen */
    ...
    .cart {
      ...
      align-items: center;
      gap: 8px;
      box-shadow: 0 2px 5px rgba(217, 53, 53, 0.6);
      user-select: none;
      transition: background-color 0.3s;
    }
    ...
    .product-card:hover {
      ...
      box-shadow: 0 10px 20px rgba(217,53,53,0.2);
    }
    ...
    .btn-add-cart {
      ...
      width: 100%;
      box-shadow: 0 4px 10px rgba(217,53,53,0.4);
    }
...
.logo {
  ...
  height: 70px; 
  cursor: pointer;
  transition: transform 1s ease; /* para hover si quieres */
}
...
<div class="container">
  ...
  <main>
    <h2>Categoría: Raquetas - Raquetball</h2>
    <div class="product-grid">
      <!-- Productos cargados dinámicamente aquí -->
      <?php foreach($products as $product): ?>
      <div class="product-card">
        <div class="fav-icon" title="Agregar a favoritos"></div>
        <img src="<?= htmlspecialchars($product['image_url'] ?? 'https://via.placeholder.com/150') ?>" alt="<?= htmlspecialchars($product['name']) ?>" />
        <div class="product-name"><?= htmlspecialchars($product['name']) ?></div>
        <div class="product-price">
          Bs. <?= number_format($product['price'], 2) ?>
          <?php if (!empty($product['old_price'])): ?>
            <del>Bs. <?= number_format($product['old_price'], 2) ?></del>
          <?php endif; ?>
        </div>
        <button class="btn-add-cart">Añadir al carrito</button>
        <div class="details-link">Ver detalles del producto</div>
      </div>
      <?php endforeach; ?>
    </div>
  </main>
